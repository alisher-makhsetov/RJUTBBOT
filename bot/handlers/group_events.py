...
from db.models import Group  # ✅ Channel → Group
...
async def user_joined_group(update: ChatMemberUpdated, bot: Bot, session: AsyncSession):
    """User guruh/kanalga qo'shilganda avtomatik warning o'chirish va start tugmasi"""
    try:
        # Database'dan required guruh/kanallarni olish
        result = await session.execute(select(Group).where(Group.is_required == True))  # ✅
        required_groups = result.scalars().all()  # ✅

        # Bu guruh/kanal required guruh/kanallardan birimi?
        is_required_group = any(group.chat_id == update.chat.id for group in required_groups)  # ✅
